...
import { registerBodegaAction, inviteTeamMemberAction, registerInvitedUserAction, RegisterActionResult, InviteMemberResult } from "@/actions/auth-actions";
...
function RegisterContent() {
  ...
  let getInviteByTokenAction: (token: string) => Promise<{ success: boolean; data?: any; error?: string }>;
  import("@/actions/invites").then((m) => { getInviteByTokenAction = m.getInviteByTokenAction; });

  useEffect(() => {
    if (!inviteToken) return;
    const load = async () => {
      if (!getInviteByTokenAction) return;
      const res = await getInviteByTokenAction(inviteToken);
      setInviteData(res.success ? res.data : null);
  ...
  const [inviteRegState, inviteRegFormAction, isInviteRegPending] = useActionState(registerInvitedUserAction, { success: false, message: "" });
  ...
  if (inviteToken && !inviteData) {
    return (
      <main className="min-h-dvh bg-gray-50 dark:bg-zinc-950 px-4 py-6 flex items-center justify-center">
        <div className="max-w-md w-full">
          <DsAlert variant="warning" message="Esta invitación es inválida o ha expirado. Pide un nuevo enlace a tu administrador." />
        </div>
      </main>
    );
  }
  ...
  if (inviteToken && inviteData) {
    return (
      <main className="min-h-dvh bg-gray-50 dark:bg-zinc-950 px-4 py-6 pb-24 flex flex-col justify-center">
        <div className="max-w-md mx-auto w-full">
          <DsCard>
            <div className="flex flex-col gap-5">
              {inviteRegState.success ? (
                <div className="flex flex-col items-center gap-4 text-center py-6">
                  <CheckCircle2 className="w-12 h-12 text-emerald-700 dark:text-emerald-400" />
                  <h2 className="text-xl font-black text-gray-900 dark:text-zinc-50 uppercase">¡YA ERES PARTE DEL EQUIPO!</h2>
                  <p className="text-sm text-gray-600 dark:text-zinc-400">{inviteRegState.message}</p>
                  <Link
                    href="/login"
                ...
                <>
                  <div className="text-center pb-4 border-b border-gray-100 dark:border-zinc-800">
                    ...
                    <h1 className="text-2xl font-black text-gray-900 dark:text-zinc-50 uppercase tracking-tight mt-2">
                      ÚNETE A {inviteData.tenantName.toUpperCase()}
                    </h1>
                    <p className="text-xs text-gray-600 dark:text-zinc-400 font-medium mt-1">
                      Te han invitado como {inviteData.role === "ADMIN" ? "Administrador" : "Cajero"}
                    </p>
                  </div>

                  {inviteRegState.message && !inviteRegState.success && (
                    <DsAlert variant="error" message={inviteRegState.message} />
                  )}

                  <form action={inviteRegFormAction} className="flex flex-col gap-4">
                    <input type="hidden" name="inviteToken" value={inviteToken} />
                    <input type="hidden" name="email" value={inviteData.email} />
                    ...
                    <DsButton type="submit" disabled={isInviteRegPending}>
                      {isInviteRegPending ? "REGISTRANDO..." : "UNIRME A LA BODEGA"}
                    </DsButton>
                    ...
                    <button
                      ...
                      className="w-full bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 py-3.5 px-4 text-sm font-bold font-mono text-gray-900 dark:text-zinc-50 flex items-center justify-center gap-3 hover:bg-gray-50 dark:hover:bg-zinc-700 active:scale-95 transition-transform"
                    >

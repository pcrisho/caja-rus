          ...
          <DsCard variant="flat" padding="lg">
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                {history.map((closure) => (
                  <div key={closure.id} className="bg-white dark:bg-zinc-900 p-4 flex flex-col gap-1">
                    <div className="flex justify-between items-center">
                      <p className="text-sm font-semibold text-gray-900 dark:text-zinc-50">
                        {new Date(closure.closedAt).toLocaleString()}
                      </p>
                      <p className={`font-bold text-sm tabular-nums ${Number(closure.difference) === 0 ? "text-emerald-700 dark:text-emerald-400" : Number(closure.difference) > 0 ? "text-amber-700 dark:text-amber-400" : "text-red-700 dark:text-red-400"}`}>

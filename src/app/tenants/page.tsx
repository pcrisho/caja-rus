...
export default function TenantsPage() {
  ...
  if (active.length === 0) {
    return (
      <main className="min-h-dvh bg-white dark:bg-zinc-950 px-4 py-6">
        <div className="mx-auto flex w-full max-w-sm flex-col gap-4">
          <div className="bg-amber-100 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-xl p-4">
            <p className="text-amber-700 dark:text-amber-400 text-base font-semibold">Sin bodega asignada</p>
            <p className="text-amber-700 dark:text-amber-400 text-base mt-1">Tu cuenta está activa pero aún no te asignaron a una bodega. Pide al administrador que te agregue.</p>
          </div>
        </div>
      </main>
    );
    ...
    <main className="min-h-dvh bg-white px-4 py-6">
      <div className="mx-auto flex w-full max-w-sm flex-col gap-4">
        ...
        <div className="space-y-3">
          {active.map((m) => (
            <a key={m.tenantId} href={getTenantLandingPath(m.tenantSlug)} className="block bg-gray-100 dark:bg-zinc-800 rounded-xl p-5 active:scale-95 transition-transform">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="rounded-full bg-white dark:bg-zinc-700 border border-gray-200 dark:border-zinc-700 px-3 py-1 text-sm font-semibold text-gray-700 dark:text-zinc-300">{m.tenantRole === 'ADMIN' ? 'Admin' : 'Cajero'}</span>
              </div>

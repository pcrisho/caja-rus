...
export function UsersClient({ tenantSlug, initialMembers }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {members.map((member) => (
        <div key={member.id} className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col gap-3 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="font-bold text-gray-900">{member.user.name || "Sin nombre"}</p>
              <p className="text-sm text-gray-500">{member.user.email}</p>
            </div>
            <span className={`px-2 py-1 rounded text-xs font-bold ${member.isActive ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
              {member.isActive ? "Activo" : "Inactivo"}
            </span>
          </div>
          
          {member.isActive && (
            <div className="flex gap-2 items-center justify-between border-t border-gray-100 pt-3 mt-1">
              <select
                value={member.role}
                onChange={(e) => handleRoleChange(member.id, e.target.value)}
                disabled={isPending}
                className="border-2 border-gray-300 rounded-xl px-3 py-2 text-sm font-semibold focus:outline-none focus:border-blue-900 focus:ring-1 focus:ring-blue-900 bg-white"
              >
              ...
              <button
                ...
                disabled={isPending}
                className="text-red-600 font-semibold text-sm bg-red-50 hover:bg-red-100 px-4 py-2 rounded-xl transition-colors disabled:opacity-50"
              >
                Desactivar
              </button>
            </div>

...
export function CsvImport({ tenantSlug }: CsvImportProps) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-sm mx-auto">
      {error && (
        <div className="bg-red-100 border border-red-200 text-red-700 p-4 rounded-xl text-base whitespace-pre-line">
          {error}
        </div>
      )}
      
      {success && (
        <div className="bg-emerald-100 border border-emerald-200 text-emerald-700 p-4 rounded-xl text-base flex items-start gap-2">
          <CheckCircle className="shrink-0 mt-0.5" size={20} />
          <span>{success}</span>
        </div>
      )}

      <div className="bg-gray-100 rounded-xl p-6 flex flex-col items-center justify-center gap-4 text-center border-2 border-dashed border-gray-300 relative">
        <input 
          type="file" 
          accept=".csv" 
        ...
        <div>
          <h3 className="text-gray-900 font-bold text-lg">Subir archivo CSV</h3>
          <p className="text-gray-500 text-sm mt-1">Toca aquí para buscar el archivo</p>
        </div>
      </div>
      ...
      {previewData.length > 0 && (
        <div className="flex flex-col gap-4">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
            <AlertTriangle className="text-amber-700 shrink-0" />
            <div className="text-amber-700 text-sm">
              Se encontraron <strong>{previewData.length} productos</strong> válidos. Revisa una muestra antes de importar.
            </div>
          </div>

          <div className="bg-gray-100 rounded-xl p-4 flex flex-col gap-3 max-h-60 overflow-y-auto">
            {previewData.slice(0, 5).map((p, i) => (
              <div key={i} className="flex justify-between items-center bg-white p-3 rounded-lg border border-gray-200">
                <div className="flex flex-col">
                  ...
                  <span className="text-gray-500 text-xs">Stock: {p.stock} {p.unitType === 'UNIT' ? 'UN' : 'KG'}</span>
                </div>
                <span className="text-emerald-700 font-bold">S/ {p.sellingPrice.toFixed(2)}</span>
              </div>
            ))}
            {previewData.length > 5 && (
              <p className="text-center text-gray-500 text-xs py-2">+ {previewData.length - 5} productos más</p>
            )}
          </div>

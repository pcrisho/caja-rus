    ...
    <div className="bg-gray-100 rounded-xl p-4 flex flex-col gap-3">
      <div className="flex justify-between items-start gap-2">
        <div>
          ...
          {product.barcode && (
            <p className="text-gray-500 text-xs mt-1">CÓDIGO: {product.barcode}</p>
          )}
        </div>
        <div className="text-right whitespace-nowrap">
          <p className="text-emerald-700 font-bold text-xl">S/ {price}</p>
          <p className="text-gray-500 text-xs">por {product.unitType === 'UNIT' ? 'UNIDAD' : 'KG'}</p>
        </div>
      </div>

      <div className="flex items-center justify-between mt-2">
        <div className="flex flex-col">
          <span className="text-gray-500 text-xs uppercase tracking-wider font-bold">Stock Actual</span>
          <span className="text-gray-900 font-bold text-lg">{stock} {product.unitType === 'UNIT' ? 'UN' : 'KG'}</span>
        </div>
      ...
      <Link
        ...
        className="mt-2 w-full bg-white border-2 border-gray-300 rounded-xl py-3 px-4 flex items-center justify-center gap-2 text-gray-900 font-semibold hover:bg-gray-50 active:scale-95 transition-transform"
      >
        <Pencil size={20} />
        EDITAR
      </Link>

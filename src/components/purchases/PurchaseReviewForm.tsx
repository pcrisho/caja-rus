...
export function PurchaseReviewForm({
  ...
  const [items, setItems] = useState<ItemState[]>(() => {
    ...
    if (rawItems.length === 0) {
      return [{
        id: crypto.randomUUID(), isNewProduct: false, productId: '', name: '',
        barcode: '', unitType: 'UNIT', sellingPrice: '', categoryId: '',
        quantity: '1', unitCost: '0.00', totalCost: '0.00',
      }];
    }
    ...
    return rawItems.map((rawItem: any) => {
      ...
      if (matchedProduct) {
        return {
          id: crypto.randomUUID(), isNewProduct: false, productId: matchedProduct.id,
          name: matchedProduct.name, barcode: matchedProduct.barcode || '',
          unitType: matchedProduct.unitType,
          sellingPrice: Number(matchedProduct.sellingPrice).toFixed(2),
          categoryId: '', quantity: qty, unitCost: uCost, totalCost: totCost,
        };
      }
      ...
      return {
        id: crypto.randomUUID(), isNewProduct: true, productId: '',
        name: extractedName, barcode: '', unitType: 'UNIT',
        sellingPrice: suggestedSellingPrice, categoryId: '',
        quantity: qty, unitCost: uCost, totalCost: totCost,
      };
    });
  });
  ...
  const handleRucChange = (ruc: string) => {
    setFormData((prev) => {
      ...
      return { ...prev, supplierRuc: ruc, supplierName: match ? match.name : prev.supplierName };
    });
  };

  const handleSelectSupplierChip = (supplier: SupplierOption) => {
    setFormData((prev) => ({ ...prev, supplierRuc: supplier.ruc, supplierName: supplier.name }));
  };
    ...
    setItems((prevItems) =>
      prevItems.map((item) => {
        ...
        if ('quantity' in updates || 'unitCost' in updates) {
          ...
          if (!('totalCost' in updates)) updated.totalCost = (q * uc).toFixed(2);
        } else if ('totalCost' in updates && !('unitCost' in updates)) {
          const q = Number(updated.quantity) || 0;
          const tc = Number(updated.totalCost) || 0;
          if (q > 0) updated.unitCost = (tc / q).toFixed(2);
        }
        return updated;
      })
    );
  ...
  const handleSelectCatalogProduct = (id: string, productId: string) => {
    ...
    if (!product) return;
    updateItem(id, {
      isNewProduct: false, productId: product.id, name: product.name,
      ...
      const purchaseItems: PurchaseItemInput[] = items.map((item) => {
        if (item.isNewProduct) {
          return {
            isNewProduct: true,
            newProductData: {
              name: item.name.trim(), barcode: item.barcode.trim() || undefined,
              sellingPrice: Number(item.sellingPrice), unitType: item.unitType,
              categoryId: item.categoryId || undefined,
            },
            quantity: Number(item.quantity), unitCost: Number(item.unitCost), totalCost: Number(item.totalCost),
          };
        }
        return {
          productId: item.productId, quantity: Number(item.quantity),
          unitCost: Number(item.unitCost), totalCost: Number(item.totalCost),
        };
      });
    ...
    <div className="flex flex-col gap-6 w-full max-w-md mx-auto">
      <div className="flex items-center gap-3">
        <button
          onClick={onCancel}
          type="button"
          className="p-2 -ml-2 text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-50 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors"
          aria-label="Volver"
        >
          <ArrowLeft size={20} />
        </button>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        <div className="bg-white dark:bg-zinc-900 p-4 flex flex-col gap-4">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                {suppliers.slice(0, 5).map((sup) => (
                  <button
                    ...
                    className="whitespace-nowrap px-3 py-1.5 text-xs font-medium bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-zinc-300 hover:bg-blue-100 dark:hover:bg-blue-900/40 hover:text-blue-900 dark:hover:text-blue-300 transition-colors"
                  >
                    {sup.name || sup.ruc}
                  </button>
                ))}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-zinc-200">RUC Proveedor</label>
              <input
                type="text"
                name="supplierRuc"
                maxLength={11}
                ...
                className="w-full bg-transparent border-b border-gray-200 dark:border-zinc-700 py-2.5 text-sm text-gray-900 dark:text-zinc-50 placeholder:text-gray-400 dark:placeholder:text-zinc-500 focus-visible:outline-none focus-visible:border-blue-900 transition-colors"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-zinc-200">Comprobante #</label>
              <input
                type="text"
                name="invoiceNumber"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-zinc-200">Razón Social / Nombre Proveedor</label>
            <input
              type="text"
              name="supplierName"
          ...
          <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-100 dark:border-zinc-800">
            <div className="flex justify-between items-center">
              <label className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-zinc-200">Monto Total Comprobante (S/)</label>
            ...
            <input
              ...
              className="w-full bg-transparent border-b border-gray-200 dark:border-zinc-700 py-3 text-2xl font-bold text-emerald-700 dark:text-emerald-400 tabular-nums focus-visible:outline-none focus-visible:border-blue-900 transition-colors"
            />
          </div>
        </div>
        ...
        <div className="flex justify-between items-center px-1">
          ...
          <button
            type="button"
            onClick={addItem}
            className="text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 text-sm font-bold flex items-center gap-1 active:scale-95 transition-transform"
          >
        ...
        <div className="flex flex-col gap-4">
          {items.map((item, index) => {
            ...
            return (
              <div key={item.id} className="bg-white dark:bg-zinc-900 p-4 flex flex-col gap-4">
                <div className="flex justify-between items-center border-b border-gray-100 dark:border-zinc-800 pb-2">
                  <span className="font-bold text-gray-900 dark:text-zinc-50 text-sm flex items-center gap-1.5">
                    <span className="w-5 h-5 bg-blue-900 dark:bg-blue-400 text-white dark:text-zinc-950 text-xs flex items-center justify-center font-bold">
                      {index + 1}
                    </span>
                    {item.isNewProduct ? (
                  ...
                  {items.length > 1 && (
                    <button
                      ...
                      className="text-red-600 dark:text-red-400 p-1.5 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"
                      title="Eliminar ítem"
                    >
                      <Trash2 size={18} />
                    </button>
                ...
                <div className="flex bg-gray-100 dark:bg-zinc-800 p-1 gap-1">
                  <button
                    type="button"
                    ...
                    className={`flex-1 py-1.5 text-xs font-bold transition-colors flex items-center justify-center gap-1 ${
                      !item.isNewProduct
                        ? 'bg-white dark:bg-zinc-900 text-blue-900 dark:text-blue-400'
                  ...
                  <button
                    ...
                    className={`flex-1 py-1.5 text-xs font-bold transition-colors flex items-center justify-center gap-1 ${
                      item.isNewProduct
                        ? 'bg-white dark:bg-zinc-900 text-amber-700 dark:text-amber-400'
                        : 'text-gray-500 dark:text-zinc-400'
                    }`}
                  >
                ...
                {!item.isNewProduct ? (
                  <div className="flex flex-col gap-2">
                    ...
                    <select
                      ...
                      className="w-full appearance-none bg-transparent border-b border-gray-200 dark:border-zinc-700 py-2.5 pr-8 text-sm text-gray-900 dark:text-zinc-50 focus-visible:outline-none focus-visible:border-blue-900 transition-colors font-semibold"
                    >
                    ...
                    {selectedProduct && (
                      <div className="bg-blue-50 dark:bg-blue-950/30 text-blue-900 dark:text-blue-300 p-2.5 text-xs flex justify-between">
                  ...
                  <div className="flex flex-col gap-3 bg-amber-50/50 dark:bg-amber-950/10 p-3">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-zinc-200">Nombre del Producto *</label>
                      <input
                        type="text"
                        placeholder="Ej. Arroz Superior 1kg"
                        value={item.name}
                        onChange={(e) => updateItem(item.id, { name: e.target.value })}
                        className="w-full bg-transparent border-b border-gray-200 dark:border-zinc-700 py-2 text-sm text-gray-900 dark:text-zinc-50 placeholder:text-gray-400 dark:placeholder:text-zinc-500 focus-visible:outline-none focus-visible:border-blue-900 transition-colors"
                      />
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-zinc-200">Unidad</label>
                        <select
                          value={item.unitType}
                          onChange={(e) => updateItem(item.id, { unitType: e.target.value as any })}
                          className="w-full appearance-none bg-transparent border-b border-gray-200 dark:border-zinc-700 py-2 pr-8 text-xs font-bold text-gray-900 dark:text-zinc-50 focus-visible:outline-none focus-visible:border-blue-900 transition-colors"
                        >
                      ...
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-zinc-200">P. Venta (S/) *</label>
                        <input
                          type="number"
                          step="0.01"
                          min="0.01"
                          ...
                          className="w-full bg-transparent border-b border-gray-200 dark:border-zinc-700 py-2 text-sm font-bold text-emerald-700 dark:text-emerald-400 tabular-nums focus-visible:outline-none focus-visible:border-blue-900 transition-colors"
                        />
                      </div>
                    </div>
                    ...
                    {categories.length > 0 && (
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-zinc-200">Categoría</label>
                        <select
                          value={item.categoryId}
                          onChange={(e) => updateItem(item.id, { categoryId: e.target.value })}
                          className="w-full appearance-none bg-transparent border-b border-gray-200 dark:border-zinc-700 py-2 pr-8 text-xs text-gray-900 dark:text-zinc-50 focus-visible:outline-none focus-visible:border-blue-900 transition-colors"
                        >
                ...
                <div className="grid grid-cols-3 gap-2.5 pt-1">
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      className="w-full bg-transparent border-b border-gray-200 dark:border-zinc-700 py-2 text-sm font-semibold text-gray-900 dark:text-zinc-50 tabular-nums focus-visible:outline-none focus-visible:border-blue-900 transition-colors"
                    />
                  </div>
                  <div className="flex flex-col gap-1">
                    <label className="text-gray-500 dark:text-zinc-400 text-xs font-bold tracking-wider uppercase truncate">
        ...
        <div className="flex flex-col gap-3 pt-2">
          <DsButton type="submit" disabled={loading} icon={<Check size={24} />}>
